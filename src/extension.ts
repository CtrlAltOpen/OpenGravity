...
class OllamaViewProvider implements vscode.WebviewViewProvider {
    ...
    private _loadHistory(): void {
        const clearAllBefore = this._context.globalState.get<number>('clearAllHistoryBefore', 0);
        const savedAt = this._context.workspaceState.get<number>('chatHistorySavedAt', 0);
        if (savedAt <= clearAllBefore) { return; }
        const saved = this._context.workspaceState.get<any[]>('chatHistory', []);
...
${customSystemPrompt ? `\nUser custom instructions:\n${customSystemPrompt}` : ''}`;
    ...
    private async _buildContextMessage(): Promise<string> {
        ...
        const allDocuments = vscode.workspace.textDocuments.filter(doc =>
            doc.uri.scheme === 'file' && !doc.fileName.includes(path.sep + 'node_modules' + path.sep)
        );
        if (allDocuments.length > 0) {
            ...
            const sorted = [...allDocuments].sort((a, b) => {
                ...
                if (activeDir) {
                    ...
                    if (aInDir !== bInDir) { return bInDir - aInDir; }
                }
                return 0;
            });
        ...
        if (gitRoot) {
            try {
                ...
                if (branch || log) {
                    let gitCtx = '';
                    if (branch) { gitCtx += `Branch: ${branch}\n`; }
                    if (log) { gitCtx += `Recent commits:\n${log}\n`; }
                    if (diffStat) { gitCtx += `Uncommitted changes:\n${diffStat}`; }
